...
|---|---|---|
| RF-01 | Register sighting | Capture photo with camera only; obtain GPS coordinates from device; collect bird name, date/time, quantity, notes, weather. Quantity integer >= 1. Photo and location required. Show field validation. Successful save persists record and returns to list. Coordinates never manually entered. |
