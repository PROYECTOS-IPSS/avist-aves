...
export function sanitizeBirdName(value: string): string {
  return Array.from(value.replace(/[^\p{L}\p{M}\p{N} #&]/gu, ""))
    .slice(0, BIRD_NAME_MAX_LENGTH)
    .join("");
}

function hasValidBirdNameCharacters(value: string): boolean {
  return (
    sanitizeBirdName(value) === value &&
    Array.from(value).length <= BIRD_NAME_MAX_LENGTH
  );
}
...
export function validateEditableDraft(
  ...
  if (!observedAt) {
    errors.observedAt =
      "Usa una fecha válida con formato AAAA-MM-DD y una hora HH:MM.";
  } else if (observedAt.getTime() > now.getTime()) {
    errors.observedAt =
      "La fecha y hora del avistamiento no pueden estar en el futuro.";
  }
...
export function validateWholeDraft(
  draft: SightingDraft,
  now = new Date(),
): ValidationErrors {
...
export function normalizeEditableDraft(
  draft: SightingDraft,
  now = new Date(),
): ValidationResult<NormalizedEditableSightingDraft> {
  const errors = validateEditableDraft(draft, now);
  const observedAt = parseDraftDateTime(draft);
